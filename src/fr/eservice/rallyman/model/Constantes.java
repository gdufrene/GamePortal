package fr.eservice.rallyman.model;

/**
 * Constantes du jeu.
 * @author rally-devteam
 */
public class Constantes {
	
	public static final String TYPE_TERRE 	= "terre";
	public static final String TYPE_FORET 	= "foret";
	public static final String TYPE_ROCHERS = "rochers";
	public static final String TYPE_HERBE 	= "herbe";
	public static final String TYPE_NEIGE 	= "neige";
	public static final String TYPE_VILLAGE = "village";
	public static final String TYPE_PLAN_EAU= "planEau";
	
	public static final int VOITURE_MAX_PAR_CELLULE = 2;
	public static final int NOMBRE_JOUEURS = 2;
	public static final int NOMBRE_COURSES = 3;
	
	public static final String DE_VITESSE1  = "vitesse1";
	public static final String DE_VITESSE2  = "vitesse2";
	public static final String DE_VITESSE3  = "vitesse3";
	public static final String DE_VITESSE4  = "vitesse4";
	public static final String DE_VITESSE5  = "vitesse5";
	public static final String DE_GAZ1 		= "gaz1";
	public static final String DE_GAZ2 		= "gaz2";
	
}
